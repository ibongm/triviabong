...
- **firestore.rules commits (Phase 1 #8; Phase 2 #4):** `firebase emulators:exec --only firestore "node <script>.mjs"` before deploying — never deploy rules straight from this plan without that run and explicit confirmation. Phase 1 #8 is committed (`b78c5d4`) but still **undeployed** as of this update — its "emulator verified" annotation was added externally and isn't independently confirmable from repo state (no test script/artifact checked in); re-verify before running `firebase deploy --only firestore:rules`.
- **Phase 2 refactor commits:** land and smoke-test individually before starting the next (they're ordered for a reason); `npm run build` clean after each. The `npm run lint` baseline this originally meant (9 problems) is stale — the codebase is now at 42 (verified 2026-08-04), almost all pre-existing debt surfaced by the Phase 2/3 refactor (unused vars from the auth-listener rewrite, `setState`-in-effect on several pre-existing effects) rather than newly introduced by any single commit; treat 42 as the current floor until a dedicated lint-cleanup commit lands, and call out in any future commit message if a change pushes the count higher.
